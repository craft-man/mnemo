...
| `superseded_without_history` | Entity or concept page has `superseded_by:` in frontmatter but no `## History` section |
...
**11c. Superseded without history** — entity or concept pages with `superseded_by:` in frontmatter but no `## History` section. Offer to insert an empty `## History` section before `## Links`.
